...
class Teachers:
    ...
    def get_labels(self, teacher_ids, sa_t_1, sa_t_2, r_t_1, r_t_2, info_t_1, info_t_2):
        ...
        # process rewards
        r_ts = [
            teacher.process_reward(
                sa_t_1[i], sa_t_2[i], r_t_1[i], r_t_2[i], info_t_1[i], info_t_2[i]
            )
            for i, teacher in enumerate(teachers)
        ]
        r_t_1 = np.concatenate([r_t[0] for r_t in r_ts], axis=1).transpose(1, 0)[
            :, :, None
        ]
        r_t_2 = np.concatenate([r_t[1] for r_t in r_ts], axis=1).transpose(1, 0)[
            :, :, None
        ]
        ...
        beta = np.array(
            [
                teacher.get_beta(sa_t_1[i], sa_t_2[i], info_t_1[i], info_t_2[i])
                for i, teacher in enumerate(teachers)
            ]
        )[:, None]
        ...
        return sa_t_1, sa_t_2, r_t_1, r_t_2, labels, teacher_ids, beta
