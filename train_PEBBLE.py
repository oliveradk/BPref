...
class Workspace(object):
    ...
    def evaluate(self):
        ...
        if self.cfg.log_episode:
            utils.log_episode(
                self.env, self.agent, self.logger._sw, f"step_{self.step}"
            )
        ...
        self.logger.log(
            "eval/true_episode_reward", average_true_episode_reward, self.step
        )
        if self.log_success:
    ...
    def learn_reward(self, first_flag=0):  # TODO: log betas of queries
        # get querries
    ...
    def run(self):
        ...
        while self.step < self.cfg.num_train_steps:
            if done:
                ...
                self.logger.log("train/beta_mean", np.mean(self.betas), self.step)
